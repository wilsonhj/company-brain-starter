---
type: company
status: living
---
# Mission and Vision

We build a durable your company business by compounding good decisions, documenting how we operate, and giving every teammate (and every AI agent) the same context the founders have.

**Tagline:** *A starter company brain*

## What we believe
- Context compounds. The more clearly we write down how we operate, the faster everyone — including our AI agents — gets.
- Decisions over opinions. We keep a written [[Decision Log]] so we can learn from our own track record.
- Focus beats breadth, especially at your team people. Our bets are deliberate; see [[Annual Strategy]].

The fast-moving version of "where we are against this" lives in [[MEMORY]]; the depth lives in [[Intelligence_snapshot]].

## Related
- [[Company Structure]]
- [[Annual Strategy]]
- [[Glossary]]
- [[Intelligence_snapshot]]
